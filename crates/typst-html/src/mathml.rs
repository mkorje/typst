...
/// zero so that there is no space between columns. We also align inline
/// multiline equations to the left.
///
...
mtable.multiline-equation mtr:not(:last-child) mtd {
  padding-bottom: 0;
}
...
fn handle_realized(
    ...
    if target.math_shift != ctx.css.math_shift {
        let value =
            if target.math_shift == MathShift::Normal { "normal" } else { "compact" };
        properties.push("math-shift", value);
    }
