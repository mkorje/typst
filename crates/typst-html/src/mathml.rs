...
impl<'v, 'e> MathContext<'v, 'e> {
    ...
    fn handle_into_nodes(
        ...
        self.handle_into_self(item, styles)?;
...
fn handle_realized(
    ...
    match &comp.kind {
        MathKind::Glyph(item) => handle_glyph(item, ctx, styles, props)?,
...
fn handle_glyph(
    ...
    let stretchy = true; //item.stretch.is_some();
...
fn handle_accent(
    ...
    let accent = ctx.handle_into_node(&item.accent, styles)?;
...
fn handle_scripts(
    ...
    macro_rules! handle {
        ($content:ident) => {
            item.$content
                .as_ref()
                .map(|x| ctx.handle_into_node(x, styles))
                .transpose()?
        };
    }
