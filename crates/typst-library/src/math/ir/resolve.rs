...
fn resolve_overline<'a, 'v, 'e>(
    ...
    let cramped = ctx.store_styles(style_cramped());
    let base = ctx.resolve_into_item(&elem.body, ctx.chain_styles(styles, cramped))?;
...
fn resolve_underoverspreader<'a, 'v, 'e>(
    ...
    let base = match position {
        Position::Under => {
            ...
            let annotation =
                ctx.resolve_into_item(annotation, bumped_styles.chain(under_style))?;
            ScriptsItem::create(
                base,
                None,
                Some(annotation),
        ...
        Position::Over => {
            ...
            let annotation =
                ctx.resolve_into_item(annotation, bumped_styles.chain(over_style))?;
            ScriptsItem::create(
                base,
                Some(annotation),
                None,
