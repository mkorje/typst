...
impl<'a> RadicalItem<'a> {
    pub fn new(
        ...
        let kind = Self { radicand, index, span };
        let props = MathProperties::default(styles);
        MathComponent { kind, props }
    }
}
...
impl<'a> MathRun<'a> {
    ...
    pub fn new(items: Vec<MathItem<'a>>) -> MathRun<'a> {
        ...
        for mut item in iter {
            ...
            if !item.is_ignorant() {
                if let Some(i) = last {
                    spacing(&mut resolved[i], space.take(), &mut item);
                }
...
fn spacing<'a>(l: &mut MathItem, space: Option<MathItem<'a>>, r: &mut MathItem) {
    use MathClass::*;
    ...
    match (l.class(), r.class()) {
        ...
        _ if (l.is_spaced() || r.is_spaced()) => {} //space.clone(),
...
fn resolve_cells<'a, 'v, 'e>(
    ...
    _span: Span,
    _children: &str,
) -> SourceResult<Vec<Vec<MathRun<'a>>>> {
    let denom_style = ctx.store_styles(style_for_denominator(styles));
    let bumped_styles = ctx.arenas.bump.alloc(styles);
    rows.iter()
        .map(|row| {
            row.iter()
                .map(|cell| ctx.resolve_into_run(cell, bumped_styles.chain(denom_style)))
...
fn resolve_root<'a, 'v, 'e>(
    ...
    ctx.push(RadicalItem::new(radicand, index, styles, elem.span()));
    Ok(())
}

fn resolve_underline(
