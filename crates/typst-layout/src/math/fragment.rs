...
impl MathFragment {
    ...
    pub fn kern_at_height(&self, corner: Corner, height: Abs) -> Abs {
        match self {
            Self::Glyph(glyph) => {
                // TODO: figure out what should happen here with glyph assemblies. Maybe use first and last based on the corner??
                kern_at_height(
                    &glyph.text.font,
                    GlyphId(glyph.text.glyphs[0].id),
                    corner,
...
impl GlyphFragment {
    pub fn new(font: &Font, styles: StyleChain, c: char, span: Span) -> Self {
        let mut tmp = [0; 4];
        Self::try_new(font, styles, c.encode_utf8(&mut tmp), span).unwrap()
    }
    ...
    pub fn update_glyph(&mut self) {
        ...
        self.width = width;
        self.ascent = ascent.at(self.text.size);
        self.descent = descent.at(self.text.size);
    ...
    pub fn into_frame(self) -> Frame {
        ...
        frame.push(Point::with_y(self.ascent + self.shift), FrameItem::Text(self.text));
        frame.modify(&self.modifiers);
        frame
    }

    /// Try to stretch a glyph to a desired height.
    pub fn stretch_vertical(&mut self, height: Abs, short_fall: Abs) {
        self.stretch(height, short_fall, Axis::Y)
    }

    /// Try to stretch a glyph to a desired width.
    pub fn stretch_horizontal(&mut self, width: Abs, short_fall: Abs) {
        self.stretch(width, short_fall, Axis::X)
    }

    /// Try to stretch a glyph to a desired width or height.
    ///
    /// The resulting frame may not have the exact desired width or height.
    pub fn stretch(&mut self, target: Abs, short_fall: Abs, axis: Axis) {
        // If the base glyph is good enough, use it.
        // TODO: there's inconsistency here with italic correction.
        ...
        let Some(construction) = glyph_construction(&font, id, axis) else {
            return;
        };
        ...
        let min_overlap = min_connector_overlap(&self.text.font)
            .unwrap_or_default()
            .at(self.text.size);
        assemble(self, assembly, min_overlap, target, axis);
    }
    ...
    pub fn align_on_axis(&mut self, align: VAlignment) {
        if self.text.glyphs.len() == 1 {
            return;
        }
        let h = self.height();
        let axis = axis_height(&self.text.font).unwrap().at(self.text.size);
        self.ascent = align.inv().position(h + axis * 2.0);
        self.descent = h - self.ascent;
        if self.text.glyphs[0].x_advance == Em::zero() {
            self.shift += self.descent;
        }
    }
}
...
fn assemble(
    ...
    match axis {
        Axis::X => base.width = full,
        Axis::Y => {
            let baseline = full;
            // full / 2.0 + axis_height(&base.text.font).unwrap().at(base.text.size);
            base.ascent = baseline;
            base.descent = full - baseline;
            base.width = glyphs
                .iter()
