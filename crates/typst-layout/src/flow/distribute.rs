...
enum TargetState<'a, 'b> {
    ...
    /// content to which the suffix of sticky blocks attaches.
    Following,
}
...
impl<'a, 'b, P> DistributionTarget<'a, 'b, P> {
    ...
    fn stop_if_reached(
        ...
        // final region would accept through its normal overflow path.
        if sticky && !fits {
...
pub fn distribute(
    ...
    let (forced, stop_point) = match distributor.run() {
        Ok(()) => (distributor.composer.work.done(), StopPoint::Unknown),
        Err(Stop::Finish(Finish::Soft(stop_point))) => (false, stop_point),
    ...
    distributor
        .finalize(region, init, forced, stop_point)
...
impl<'a, 'b, 'x, 'y, 'z, P> Distributor<'a, 'b, 'x, 'y, 'z, P> {
    ...
    fn finalize(
        ...
        if forced {
            ...
            if let Some(snapshot) = self.sticky.take()
                && self.should_restore_sticky(&snapshot, stop_point)?
            {
    ...
    /// region can improve the layout.
    ///
    /// Migrating is the conservative default; it is only skipped when the
    /// simulated layout of the next region can be *trusted* to show that
    /// migration does not help. Any case the simulation cannot decide keeps
    /// the migration.
    fn should_restore_sticky(
        ...
        // simulation; without one, migrate conservatively.
        let StopPoint::Child = stop_point else {
            return Ok(true);
        };

        // The flow is exhausted, so there is no attached content whose fit the
        // simulation could evaluate.
        let Some(target) = self.composer.work.head() else {
            return Ok(true);
        };

        let mut next = self.regions;
        next.next();

        // A failed simulation only rules out migration to the immediate next
        // region. If a later region may improve the fit, migrate so the sticky
        // suffix can pass through the immediate region.
        if next.may_progress() {
            return Ok(true);
        }

        self.composer
            .simulate_sticky_migration(snapshot.work.clone(), target, next)
    }
